...
static BOOL CALLBACK DlgProcCluiOpts(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		CheckDlgButton(hwndDlg, IDC_NOBORDER, DBGetContactSettingByte(NULL, "CList", "NoBorder", 0) ? BST_CHECKED : BST_UNCHECKED);
		EnableWindow(GetDlgItem(hwndDlg, IDC_NOBORDER), !IsDlgButtonChecked(hwndDlg, IDC_SHOWCAPTION));
		return TRUE;
	...
	case WM_NOTIFY:
		if (((LPNMHDR) lParam)->code == PSN_APPLY) {
			...
			DBWriteContactSettingByte(NULL, "CList", "NoBorder", (BYTE) IsDlgButtonChecked(hwndDlg, IDC_NOBORDER));
			{
				TCHAR title[256];
			...
			if (IsDlgButtonChecked(hwndDlg, IDC_NOBORDER) && !IsDlgButtonChecked(hwndDlg, IDC_SHOWCAPTION))
				SetWindowLong(pcli->hwndContactList, GWL_STYLE,
					GetWindowLong(pcli->hwndContactList, GWL_STYLE) & ~(WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_POPUPWINDOW | WS_THICKFRAME));
			else
				SetWindowLong(pcli->hwndContactList, GWL_STYLE,
					GetWindowLong(pcli->hwndContactList, GWL_STYLE) | WS_THICKFRAME);
